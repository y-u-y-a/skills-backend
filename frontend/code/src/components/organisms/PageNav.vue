  ...
  <div class="page-nav contents">
    <ul class="row align-items-end">
      <li v-for="n in 20" :key="n">
        <template v-if="n == 2">
          <span>編集中</span>
          <CircleItem :text="n" :is_active="true" />
        </template>
        <template v-else>
          <CircleItem :text="n" :is_active="false" />
        </template>
      </li>
    </ul>
    <NavList>
      <template v-slot:content>
        <TextButton button_name="共有リンク取得" class="pc ml-05" />
        <TextButton button_name="ダウンロード" class="pc ml-05" />
